...
class Tree {
 ...
 public:
  ...
  Tree(char *pre_order, char *in_order, int N) {
    root = deduce_tree(pre_order, in_order, 0, N-1);
  }
  ...
  // Used variables start and end to make recursive calls by providing the start and end point of consideration
  struct Node *deduce_tree(char *pre_order, char *in_order, int start, int end);
  ...
  void traverse(TraversalType_e tt, Node* ptr);
};
